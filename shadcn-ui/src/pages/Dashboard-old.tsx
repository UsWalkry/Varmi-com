...
export default function Dashboard() {
  ...
  // Helper: find listing from loaded cache first, then local storage as fallback
  const findListingById = (id: string) => {
  ...
  useEffect(() => {
    ...
    if (authLoading) return; // Auth yüklenirken bekle
    ...
    const loadDashboardData = async () => {
      try {
        // Get all listings and user listings from MySQL API
        ...
        // TODO: Third-party orders need MySQL API implementation
        setMyPurchases([]);
        setMyTpSales([]);

        console.log('🔍 Dashboard - User Listings Debug:');
        console.log('userId:', user.id);
        console.log('allListings count:', allListings.length);
        console.log('userListings count:', userListings.length);
        console.log('userOffers count:', userOffers.length);
  ...
  const stats = useMemo(() => {
    if (!user) return { totalListings: 0, totalOffers: 0, totalFavorites: 0, unreadMessages: 0 };

    return {
      totalListings: myListings.length,
      totalOffers: myOffers.length,
      totalFavorites: favoriteListings.length,
      unreadMessages: 0 // TODO: Implement unread messages in MySQL API
    };
  }, [user, myListings.length, myOffers.length, favoriteListings.length]);
  ...
  const refreshSellerOffer = async (offerId: string) => {
    try {
      const updatedOffers = await mysqlAPI.getMyOffers();
      const fresh = updatedOffers.find(o => o.id === offerId);
      if (fresh) setSellerDialogOffer(fresh);
      setMyOffers(updatedOffers);
    } catch (error) {
      console.error('Error refreshing seller offer:', error);
    }
  };
  ...
  const sendAutoMessageToOwner = async (offer: Offer, text: string) => {
    ...
    try {
      // TODO: Implement messaging in MySQL API
      console.log('Sending message to owner:', text);
    } catch (e) {
  ...
  const sendAutoMessageToSeller = async (offer: Offer, text: string) => {
    ...
    try {
      // TODO: Implement messaging in MySQL API
      console.log('Sending message to seller:', text);
    } catch (e) {
  ...
  const handleSelectCarrier = async (offer: Offer) => {
    ...
    try {
      // TODO: Implement carrier selection in MySQL API
      await refreshSellerOffer(offer.id);
      toast.success('Kargo seçildi');
    } catch (error) {
      console.error('Error selecting carrier:', error);
      toast.error('Kargo seçimi başarısız');
    }
  };
  ...
  const handleMarkShipped = async (offer: Offer) => {
    ...
    try {
      // TODO: Implement shipping marking in MySQL API
  ...
  const handleMarkCompleted = async (offer: Offer) => {
    try {
      // TODO: Implement completion marking in MySQL API
  ...
  // TP seller handlers - simplified for MySQL API
  const refreshTpSellerOrder = async (orderId: string) => {
    // TODO: Implement third-party order refresh in MySQL API
    console.log('Refreshing TP seller order:', orderId);
  };
  ...
  const sendMessageSellerToTpBuyer = (order: ThirdPartyOrder, text: string) => {
    ...
    try {
      // TODO: Implement messaging in MySQL API
      console.log('Sending message to TP buyer:', text);
    } catch (e) {
  ...
  const handleSelectCarrierTP = (order: ThirdPartyOrder) => {
    const c = carriers.find(c => c.id === selectedCarrier);
    if (!c) return;
    // TODO: Implement TP order carrier selection in MySQL API
    toast.success('Kargo seçildi');
  };
  ...
  const handleMarkShippedTP = (order: ThirdPartyOrder) => {
    const tn = trackingNo.trim();
    if (!tn) return;
    // TODO: Implement TP order shipping in MySQL API
  ...
  const handleMarkCompletedTP = (order: ThirdPartyOrder) => {
    // TODO: Implement TP order completion in MySQL API
  ...
  const handleWithdrawOffer = async (offerId: string) => {
    ...
    try {
      ...
      const updatedOffers = await mysqlAPI.getMyOffers();
      const allOffers = await mysqlAPI.getOffers();
      const offersOnMyListings = allOffers.filter(o => 
        o.sellerId !== user.id && 
        myListings.some(l => l.id === o.listingId)
      );
      setMyOffers(updatedOffers);
      setIncomingOffers(offersOnMyListings);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog open={orderDialogOpen} onOpenChange={(v) => { if (!v) { setOrderDialogOpen(false); setOrderDialogOffer(null); } }}>
        <DialogContent>
          ...
          {orderDialogOffer && (() => {
            const listing = findListingById(orderDialogOffer.listingId);
            const acceptedAt = orderDialogOffer.acceptedAt ? formatDate(orderDialogOffer.acceptedAt) : '-';
            // 5 aşamalı dinamik akış (alıcı görünümü)
            const steps = [
              { id: 1, title: 'Sipariş Alındı', description: `Tarih: ${acceptedAt}` },
              { id: 2, title: 'Kargo Firması Seçildi', description: orderDialogOffer.shippingCarrierName ? `${orderDialogOffer.shippingCarrierName}${orderDialogOffer.shippingExtraFee && orderDialogOffer.shippingExtraFee > 0 ? ` • Ek ücret: ${formatPrice(orderDialogOffer.shippingExtraFee)}` : ''}` : 'Satıcı firma seçecek.' },
              ...
              { id: 5, title: 'Tamamlandı & Ödeme Aktarıldı', description: orderDialogOffer.completedAt ? `Tarih: ${formatDate(orderDialogOffer.completedAt)}` : 'Ödeme aktarımı bekleniyor.' }
            ];
            ...
            const handleBuyerConfirmDelivered = async () => {
              if (!orderDialogOffer) return;
              try {
                // TODO: Implement delivery confirmation in MySQL API
                await sendAutoMessageToSeller(orderDialogOffer, 'Siparişi teslim aldım. Teşekkürler.');
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {(() => {
                      ...
                      return (
                        <>
                          Teklif: {formatPrice(orderDialogOffer.price)} × {u} = {formatPrice(subtotal)}
                          {orderDialogOffer.shippingCost > 0 ? ` + ${formatPrice(orderDialogOffer.shippingCost)} kargo` : ''}
                        </>
                      );
                    })()}
                  </div>
      ...
      <Dialog open={sellerDialogOpen} onOpenChange={(v) => { if (!v) { setSellerDialogOpen(false); setSellerDialogOffer(null); setTrackingNo(''); } }}>
        <DialogContent>
          ...
          {sellerDialogOffer && (() => {
            const listing = findListingById(sellerDialogOffer.listingId);
      ...
      <Dialog open={tpBuyerDialogOpen} onOpenChange={(v) => { if (!v) { setTpBuyerDialogOpen(false); setTpBuyerOrder(null); } }}>
        <DialogContent>
          ...
          {tpBuyerOrder && (() => {
            const listing = findListingById(tpBuyerOrder.listingId);
            const steps = [
      ...
      <Dialog open={tpSellerDialogOpen} onOpenChange={(v) => { if (!v) { setTpSellerDialogOpen(false); setTpSellerOrder(null); setTrackingNo(''); } }}>
        <DialogContent>
          ...
          {tpSellerOrder && (() => {
            const listing = findListingById(tpSellerOrder.listingId);
            const steps = [
              ...
              <CardContent>
                {myListings.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    {supabaseEnabled() && !hasSupabaseSession ? (
                      <>
                        <h3 className="text-lg font-semibold mb-2">Giriş gerekli</h3>
                        <p className="text-muted-foreground mb-4">
                          İlanlarını görebilmek için lütfen giriş yap. Oturum açıldıktan sonra bu sayfayı yenile.
                        </p>
                        <Button onClick={() => navigate('/')}>Giriş Ekranına Git</Button>
                      </>
                    ) : (
                      <>
