...
import { Plus, Minus, CreditCard, Shield, Truck } from 'lucide-react';
...
export default function Checkout() {
  ...
  // Get available stock for quantity selector
  const availableStock = useMemo(() => {
    if (!offer) return 1;
    const totalStock = offer.quantity || 1;
    const soldQuantity = offer.soldToOthers || 0;
    return Math.max(1, totalStock - soldQuantity);
  }, [offer]);
  ...
  useEffect(() => {
    const loadOffer = async () => {
      ...
      try {
        // MySQL API'den teklifi direkt ID ile çek
        const offerResponse = await mysqlAPI.getOfferById(offerId);
        console.log('🔍 Checkout: Offer response:', offerResponse);
        ...
        const mappedOffer: Offer = {
          ...
          images: foundOffer.images ? JSON.parse(foundOffer.images) : [],
          soldToOthers: parseInt(foundOffer.sold_to_others) || 0
        };
        
        setOffer(mappedOffer);
        
        // İlanı da MySQL'den çek
        const listingResponse = await mysqlAPI.getListingById(mappedOffer.listingId);
        if (listingResponse.success && listingResponse.listing) {
          const l = listingResponse.listing;
